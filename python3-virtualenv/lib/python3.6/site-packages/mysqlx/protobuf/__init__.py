...
_SERVER_MESSAGES_TUPLES = (
    ...
    ("Mysqlx.ServerMessages.Type.CONN_CAPABILITIES", "Mysqlx.Connection.Capabilities"),
    (
    ...
    ("Mysqlx.ServerMessages.Type.RESULTSET_FETCH_DONE", "Mysqlx.Resultset.FetchDone"),
    (
    ...
    ("Mysqlx.ServerMessages.Type.SQL_STMT_EXECUTE_OK", "Mysqlx.Sql.StmtExecuteOk"),
    (
        "Mysqlx.ServerMessages.Type.RESULTSET_FETCH_DONE_MORE_OUT_PARAMS",
        "Mysqlx.Resultset.FetchDoneMoreOutParams",
    ),
    ("Mysqlx.ServerMessages.Type.COMPRESSION", "Mysqlx.Connection.Compression"),
)
...
try:
    ...
    for key, val in mysqlx_expect_pb2.Open.Condition.ConditionOperation.items():
        _MESSAGES[
            "Mysqlx.Expect.Open.Condition.ConditionOperation.{0}" "".format(key)
        ] = val
    for key, val in mysqlx_expect_pb2.Open.Condition.Key.items():
        _MESSAGES["Mysqlx.Expect.Open.Condition.Key.{0}" "".format(key)] = val
    ...
    for key, val in mysqlx_notice_pb2.SessionStateChanged.Parameter.items():
        _MESSAGES[
            "Mysqlx.Notice.SessionStateChanged.Parameter.{0}" "".format(key)
        ] = val

    # Mysql.Prepare
    for key, val in mysqlx_prepare_pb2.Prepare.OneOfMessage.Type.items():
        _MESSAGES["Mysqlx.Prepare.Prepare.OneOfMessage.Type.{0}" "".format(key)] = val
    ...
    if not HAVE_MYSQLXPB_CEXT:
        raise ImportError(
            "Protobuf is not available: {}" "".format(HAVE_PROTOBUF_ERROR)
        )
