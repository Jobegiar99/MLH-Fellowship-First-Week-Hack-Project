...
class FilterableStatement(Statement):
    ...
    def __init__(self, target, doc_based=True, condition=None):
        super(FilterableStatement, self).__init__(target=target, doc_based=doc_based)
        self._binding_map = {}
        self._bindings = {}
        self._having = None
        self._grouping_str = ""
...
class SelectStatement(ReadStatement):
    ...
    def get_sql(self):
        ...
        group_by = (
            " GROUP BY {0}".format(self._grouping_str) if self.has_group_by else ""
        )
        having = " HAVING {0}".format(self._having) if self.has_having else ""
        order_by = " ORDER BY {0}".format(self._sort_str) if self.has_sort else ""
        limit = (
            " LIMIT {0} OFFSET {1}".format(self._limit_row_count, self._limit_offset)
            if self.has_limit
            else ""
        )
        stmt = (
...
class UpdateStatement(FilterableStatement):
    ...
    def set(self, field, value):
        ...
        """
        self._update_ops[field] = UpdateSpec(
            mysqlxpb_enum("Mysqlx.Crud.UpdateOperation.UpdateType.SET"), field, value
        )
        self._changed = True
        return self
...
class RemoveStatement(FilterableStatement):
    ...
    def __init__(self, collection, condition):
        super(RemoveStatement, self).__init__(target=collection, condition=condition)
