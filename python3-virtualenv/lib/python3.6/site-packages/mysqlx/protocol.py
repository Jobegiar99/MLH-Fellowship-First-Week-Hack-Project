...
class Protocol(object):
    ...
    def _create_any(self, arg):
        ...
        if isinstance(arg, str):
            ...
            if arg < 0:
                return Message(
                    "Mysqlx.Datatypes.Any", type=1, scalar=build_int_scalar(arg)
                )
            return Message(
                "Mysqlx.Datatypes.Any", type=1, scalar=build_unsigned_int_scalar(arg)
            )
    ...
    def _get_binding_args(self, stmt, is_scalar=True):
        ...
        for name, value in bindings.items():
            if name not in binding_map:
                raise ProgrammingError(
                    "Unable to find placeholder for " "parameter: {0}".format(name)
                )
    ...
    def get_capabilites(self):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.CON_CAPABILITIES_GET"), msg
        )
        msg = self._reader.read_message()
    ...
    def set_capabilities(self, **kwargs):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.CON_CAPABILITIES_SET"), msg
        )
    ...
    def send_auth_start(self, method, auth_data=None, initial_response=None):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.SESS_AUTHENTICATE_START"), msg
        )
    ...
    def send_auth_continue(self, auth_data):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.SESS_AUTHENTICATE_CONTINUE"), msg
        )
    ...
    def send_prepare_prepare(self, msg_type, msg, stmt):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.PREPARE_PREPARE"), msg_prepare
        )
    ...
    def send_prepare_execute(self, msg_type, msg, stmt):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.PREPARE_EXECUTE"), msg_execute
        )
    ...
    def send_prepare_deallocate(self, stmt_id):
        ...
        self._writer.write_message(
            mysqlxpb_enum("Mysqlx.ClientMessages.Type.PREPARE_DEALLOCATE"), msg_dealloc
        )
        self.read_ok()
    ...
    def send_msg_without_ps(self, msg_type, msg, stmt):
        ...
        if stmt.has_limit:
            ...
            msg["limit"] = msg_limit
        is_scalar = (
            False if msg_type == "Mysqlx.ClientMessages.Type.SQL_STMT_EXECUTE" else True
        )
    ...
    def build_find(self, stmt):
        ...
        collection = Message(
            "Mysqlx.Crud.Collection", name=stmt.target.name, schema=stmt.schema.name
        )
