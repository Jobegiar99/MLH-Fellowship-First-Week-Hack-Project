...
from .protocol import Protocol, MessageReader, MessageWriter, HAVE_LZ4, HAVE_ZSTD
...
class SocketStream(object):
    ...
    def set_ssl(
        self, ssl_protos, ssl_mode, ssl_ca, ssl_crl, ssl_cert, ssl_key, ssl_ciphers
    ):
        """Set SSL parameters.
        ...
        if ssl_mode == SSLMode.VERIFY_IDENTITY:
            ...
            if not match_found:
                ...
                raise InterfaceError(
                    "Unable to verify server identity: {}" "".format(", ".join(errs))
                )
        ...
        for router in self._routers:
            ...
            if priority is None:
                ...
                raise ProgrammingError(
                    "The priorities must be between 0 and " "100", 4007
                )
    ...
    def __init__(self, settings):
        ...
        if "host" in settings and settings["host"]:
            self._routers.append(
                {"host": settings.get("host"), "port": settings.get("port", None)}
            )
...
class ConnectionPool(queue.Queue):
    ...
    def add_connection(self, cnx=None):
        ...
        if not cnx:
            ...
            ver = cnx.sql('show variables like "version"').execute().fetch_all()[0][1]
            if tuple([int(n) for n in ver.split("-")[0].split(".")]) > (8, 0, 10):
                cnx.sql(
                    "set mysqlx_wait_timeout = {}" "".format(self.max_idle_time)
                ).execute()
...
class PoolsManager(object):
    ...
    def _get_connections_settings(self, settings):
        ...
        for router in routers:
            ...
            connections_settings.append(
                (generate_pool_name(**connection_settings), connection_settings)
            )
    ...
    def get_connection(self, settings):
        ...
        def set_mysqlx_wait_timeout(cnx):
            ...
            if tuple([int(n) for n in ver.split("-")[0].split(".")]) > (8, 0, 10):
                cnx.sql(
                    "set mysqlx_wait_timeout = {}" "".format(pool.max_idle_time)
                ).execute()
...
class Session(object):
    ...
    def __init__(self, settings):
        ...
        if settings.get("host") and settings.get("dns-srv"):
            ...
            try:
                ...
                raise InterfaceError(
                    "Unable to locate any hosts for '{0}'" "".format(settings["host"])
                )
        ...
        if "connection-attributes" in self._settings:
            for attr_name in self._settings["connection-attributes"]:
                ...
                if not isinstance(attr_name, str):
                    raise InterfaceError(
                        "Attribute name '{}' must be a string " "type".format(attr_name)
                    )
    ...
    def rollback_to(self, name):
        ...
        if not isinstance(name, str) or len(name.strip()) == 0:
            raise ProgrammingError("Invalid SAVEPOINT name")
        self._connection.execute_nonquery(
            "sql", "ROLLBACK TO SAVEPOINT {0}" "".format(quote_identifier(name)), True
        )
    ...
    def release_savepoint(self, name):
        ...
        if not isinstance(name, str) or len(name.strip()) == 0:
            raise ProgrammingError("Invalid SAVEPOINT name")
        self._connection.execute_nonquery(
            "sql", "RELEASE SAVEPOINT {0}" "".format(quote_identifier(name)), True
        )
