...
def _validate_settings(settings):
    ...
    if "ssl-mode" in settings:
        ...
        if settings["ssl-mode"] == SSLMode.DISABLED and any(
            key in settings for key in _SSL_OPTS
        ):
    ...
    if "priority" in settings and settings["priority"]:
        try:
            ...
            if settings["priority"] < 0 or settings["priority"] > 100:
                raise ProgrammingError(
                    "Invalid priority value, " "must be between 0 and 100", 4007
                )
        except NameError:
...
def _validate_tls_versions(settings):
    ...
    if tls_versions == ["TLSv1.3"] and not TLS_V1_3_SUPPORTED:
        raise InterfaceError(TLS_VER_NO_SUPPORTED.format(tls_versions, TLS_VERSIONS))
...
def get_client(connection_string, options_string):
    ...
    if "pooling" in options_dict:
        ...
        if len(options_dict) > 0:
            raise InterfaceError(
                "Unrecognized connection options: {}" "".format(options_dict.keys())
            )
