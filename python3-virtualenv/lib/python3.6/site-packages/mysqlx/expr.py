...
class ExprParser(object):
    ...
    def assert_cur_token(self, token_type):
        ...
        if self.tokens[self.pos].token_type != token_type:
            raise ValueError(
                ...
                "type {2}, on tokens {3}"
                "".format(token_type, self.pos, self.tokens[self.pos], self.tokens)
            )
    ...
    def cast_data_type(self):
        ...
        if token.token_type in (
            ...
            return "{0}{1}".format(token.value, dimension) if dimension else token.value
    ...
    def add_sub_interval(self):
        ...
        if self.cur_token_type_in(
            ...
            if not self.cur_token_type_in(*_INTERVAL_UNITS):
                raise ValueError(
                    "Expected interval type at position {0}" "".format(self.pos)
                )
    ...
    def mul_div_expr(self):
        return self.parse_left_assoc_binary_op_expr(
            set([TokenType.MUL, TokenType.DIV, TokenType.MOD]), self.add_sub_interval
        )
