...
class MySQLConnectionAbstract(object):
    ...
    def _validate_tls_ciphersuites(self):
        ...
        if isinstance(tls_cs, str):
            if not (tls_cs.startswith("[") and tls_cs.endswith("]")):
                raise AttributeError(
                    "tls_ciphersuites must be a list, " "found: '{}'".format(tls_cs)
                )
            else:
    ...
    def _validate_tls_versions(self):
        ...
        if isinstance(tls_version, str):
            if not (tls_version.startswith("[") and tls_version.endswith("]")):
                raise AttributeError(
                    "tls_versions must be a list, found: '{}'" "".format(tls_version)
                )
            else:
        ...
        if tls_versions == ["TLSv1.3"] and not TLS_V1_3_SUPPORTED:
            raise AttributeError(TLS_VER_NO_SUPPORTED.format(tls_version, TLS_VERSIONS))
        tls_versions.sort()
        ...
        if set_ssl_flag:
            ...
            if "tls_versions" in self._ssl and self._ssl["tls_versions"] is not None:
                if self._ssl_disabled:
                    raise AttributeError(
                        "The tls_versions option can not be "
            ...
            if (
                ...
                if self._ssl_disabled:
                    raise AttributeError(
                        "The tls_ciphersuites option can not "
    ...
    def cmd_change_user(self, username="", password="", database="", charset=45):
