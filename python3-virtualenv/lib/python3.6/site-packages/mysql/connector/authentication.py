...
class MySQLLdapSaslPasswordAuthPlugin(BaseAuthPlugin):
    ...
    def _second_message(self):
        ...
        _LOGGER.debug("salted_password: %s", b64encode(salted_password).decode())
        ...
        _LOGGER.debug("client_signature: %s", b64encode(client_signature).decode())
    ...
    def _validate_first_reponse(self, servers_first):
        ...
        if (
            ...
            raise errors.InterfaceError(
                "Incomplete reponse from the server: {}" "".format(servers_first)
            )
